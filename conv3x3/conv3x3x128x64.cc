...
int main(){
    ...
    for(int i = 0; i < CHANNELS; i++){
        for(int j = 0; j < HEIGHT; j++){
            for(int k = 0; k < WIDTH; k++){
                input[i*padded_height*padded_width + (j + PADDING)*padded_width + (k + PADDING)] = (++num);
            }
        }
    }
    num=0;
        ...
        for(int ch = 0; ch < CHANNELS; ch++){
            for(int i = 0; i < KERNEL_HEIGHT; i++){
                for(int j = 0; j < KERNEL_WIDTH; j++){
                    filters[f*CHANNELS*KERNEL_HEIGHT*KERNEL_WIDTH + ch*KERNEL_HEIGHT*KERNEL_WIDTH + i*KERNEL_WIDTH + j] = (++num);
                }
            }
        }
    }
    ...
    cout << "Output shape : [ " << NUM_FILTERS << " ][ " << output_height << " ][ " << output_width << " ]" << endl;
                ...
                for(int ch = 0; ch < CHANNELS; ch++){
                    for(int k = 0; k < KERNEL_HEIGHT; k++){
                        for(int l = 0; l < KERNEL_WIDTH; l++){
                            ...
                            output += input[ch*padded_height*padded_width + input_row*padded_width + input_col] * 
                                      filters[f*CHANNELS*KERNEL_HEIGHT*KERNEL_WIDTH + ch*KERNEL_HEIGHT*KERNEL_WIDTH + k*KERNEL_WIDTH + l];
                        }
                    }
                }
                output_matrix[f*output_height*output_width + i*output_width + j] = output;
            }
        }
    }
    
    cnpy::npy_save("conv64x128x3x3.npy", output_matrix, {1,NUM_FILTERS,(unsigned long)output_height,(unsigned long)output_width}, "w");
