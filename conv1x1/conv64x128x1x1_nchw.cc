...
void conv1x1(float ***nchw_input, float ***output, float ****filters,
  ...
  float ***padded_input_nchw = new float **[input_channels];
  for (int i = 0; i < input_channels; i++) {
    padded_input_nchw[i] = new float *[padded_height];
    for (int j = 0; j < padded_height; j++) {
      padded_input_nchw[i][j] = new float[padded_width];
    }
  }

  // pad the nchw_input if PADDING is not 0
  for (int i = 0; i < input_channels; i++) {
    for (int j = 0; j < padded_height; j++) {
      for (int k = 0; k < padded_width; k++) {
        if (j < padding || j >= padded_height - padding || k < padding ||
            k >= padded_width - padding) {
          padded_input_nchw[i][j][k] = 0;
        } else {
          padded_input_nchw[i][j][k] = nchw_input[i][j - padding][k - padding];
        }
      }
    }
  }
        ...
        for (int ch = 0; ch < input_channels; ch++) {
          for (int k = 0; k < kernel_height; k++) {
            for (int l = 0; l < kernel_width; l++) {
              ...
              sum += (padded_input_nchw[ch][input_row][input_col]) *
                     (filters[f][ch][k][l]);
            }
          }
        }
  ...
  float *flattend_output =
      new float[output_filters * output_height * output_width];
  int offset = 0;
  for (int i = 0; i < output_filters; i++) {
    for (int j = 0; j < output_height; j++) {
      std::memcpy(flattend_output + offset, output[i][j],
  ...
  for (int i = 0; i < input_channels; i++) {
    for (int j = 0; j < padded_height; j++) {
      delete[] padded_input_nchw[i][j];
    }
    delete[] padded_input_nchw[i];
  }
}

int main() {
  //  nchw_input  : 1x128x224x224 (nchw)
  ...
  conv1x1(nchw_input, output_matrix, filters, bias, HEIGHT, WIDTH, CHANNELS,
          NUM_FILTERS, KERNEL_HEIGHT, KERNEL_WIDTH, STRIDE, PADDING);

  for (int i = 0; i < CHANNELS; i++) {
    for (int j = 0; j < HEIGHT; j++) {
      delete[] nchw_input[i][j];
    }
    delete[] nchw_input[i];
  }
  for (int i = 0; i < NUM_FILTERS; i++) {
